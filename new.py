...
import mongodb_db as db  # Using MongoDB database instead of SQLite
...
try:
    ...
    st.warning("NumPy and pandas are not installed. Some features may be limited.")
...
if not api_key:
    st.error("""
    Please set your GEMINI_API_KEY in one of these ways:
...
try:
    ...
    Please set your MONGODB_URI in one of these ways:
    ...
    2. In local development:
       ...
       - Or set environment variable:
         - Windows: set MONGODB_URI=your_mongodb_connection_string
         - Linux/Mac: export MONGODB_URI=your_mongodb_connection_string
    """)
    st.stop()
